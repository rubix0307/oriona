...
class WebTextServiceCore:
    ...
    def submit(self, article: Article) -> ArticleWebTextAnalysis:
        uid = self.service.submit_text(
            text=article.content,
        )
        article_text = ArticleWebTextAnalysis.objects.update_or_create(article_id=article.id, uid=uid)
        return article_text

    def submit_many(self, articles: Iterable[Article]) -> list[ArticleWebTextAnalysis]:
        submited_articles = []
    ...
    def get_result(self, submited_article: ArticleWebTextAnalysis) -> ArticleWebTextAnalysis:
        result = self.service.wait_result(
            uid=submited_article.uid,
            jsonvisible='detail',
        )
        article_text = ArticleWebTextAnalysis.from_service(article_id=submited_article.article.id, result=result)
        return article_text

    def get_results(self, submited_articles: Iterable[ArticleWebTextAnalysis]) -> list[ArticleWebTextAnalysis]:
        results = []
